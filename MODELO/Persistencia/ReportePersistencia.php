...
class ReportePersistencia {
    ...
    public function guardarReporte($reporte) {
        try {
            ...
            $sql = "INSERT INTO reportes (id, tipo, fecha_inicio, fecha_fin, id_usuario, fecha_generacion, datos) 
                    VALUES (?, ?, ?, ?, ?, ?, ?)";
     ...
     * 
     * @param string|int $idUsuario ID del usuario
     * @return array Array de reportes
     */
    public function listarReportesPorUsuario($idUsuario) {
        try {
            // Preparar la consulta SQL
            $sql = "SELECT * FROM reportes WHERE id_usuario = ? ORDER BY fecha_generacion DESC";
            ...
            // Registrar el error
            error_log("Error al listar reportes por usuario: " . $e->getMessage());
            return [];
        }
    }
